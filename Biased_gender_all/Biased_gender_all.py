...
#########################################
os.chdir('C:/Users/e0729936/source/repos/Recovery_drift_bias_ndt/Biased_gender_all')
###########################################
'''
...
def callback_gender_t(xk):
    with open('data/estimate_gender_text.txt', 'a') as f:
            f.write('   '.join(map(str, xk)) + '\n')
 
def callback_gender_v(xk):
    with open('data/estimate_gender_video.txt', 'a') as f:
            f.write('   '.join(map(str, xk)) + '\n')

#####################################################
df_gender_t = pd.read_csv('data/gender_text.csv')
df_gender_v = pd.read_csv('data/gender_video.csv')

RT_gender_t = df_gender_t['RT'].to_numpy()
R_gender_t  = df_gender_t['R'].to_numpy()
RT_gender_v = df_gender_v['RT'].to_numpy()
...
filename_gender_t = 'data/likelihood_gender_text.txt'
filename_gender_v = 'data/likelihood_gender_video.txt'
...
est_file_path_text = 'data/estimate_gender_text.txt'
...
sim_responses1 = Model_utility.simulate(result1,df_gender_t, dt, ndt_t, max_time_t)
#%%
simdata1 = {'RT':  sim_responses1[1], 'R':  sim_responses1[0]}
simdf1 = pd.DataFrame(simdata1)
simdf1.to_csv('data/simdata_text.csv', index=False)
...
sim_responses2 = Model_utility.simulate(result2,df_gender_v, dt, ndt_v, max_time_v)
#%%
simdata2 = {'RT':  sim_responses2[1], 'R':  sim_responses2[0]}
simdf2 = pd.DataFrame(simdata2)
simdf2.to_csv('data/simdata_video.csv', index=False)
